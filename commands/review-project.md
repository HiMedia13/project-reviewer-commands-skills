---
description: Qualitative multi-agent project review — scanner then 5 agents in parallel (tech-stack + architecture + 3 per-file criteria) then evaluator — producing a project-level tech-stack-fit assessment plus per-file findings and a self-contained HTML report. Does not build/run/test code.
...
3. HTML-escape every LLM string. Fill
   ...
   Build the architecture tokens: `{{ARCH_SUMMARY}}` = HTML-escaped verified `summary` (or `정보 없음` if no architecture); `{{ARCH_SCORE}}` = verified `arch_score` integer (or `N/A`); `{{ARCH_DATA_JSON}}` = the verified `architecture` object serialized as compact JSON with every `<` replaced by the JSON unicode escape `\u003c` (a `<script>` element is raw text, so HTML entities are NOT decoded there; `\u003c` cannot form `</script>` and `JSON.parse` decodes it back to `<`), or `{}` if there is no architecture. Do NOT HTML-escape inside `{{ARCH_DATA_JSON}}` — it is JSON read by `JSON.parse(textContent)`, not HTML; the report JS renders every field via `textContent`. Apply the Korean Localization display mapping to the other sections as before; `architecture` free-text is already Korean from the agent.
4. 터미널 요약을 한국어로 출력(세 축 분리): 먼저 기술 스택 적합성 헤드라인
